...
% 1) handling of extreme class imbalance (one example in one class)  
% 2) can also produce class information entropy (Fayyad et al)
%
% DESCRIPTION
...
for classstate=1:c
    tempmat = zeros(n,size(MatrixIn,2));
    for j = 1:size(tempmat,2)
        tempmat(:,j) = hist(MatrixIn(class==classstate,j),1:n)/rows;
    end
    Pcv(classstate,:,:) = tempmat;
end
...
miterms_CIE=Pcv.*(log2(Pcv)-log2(Pv)); % The term inside the log above...
miterms_CIE(isnan(miterms_CIE))=0; % with all the 0 log 0 entries removed


% Do the double summation and squeeze the unused dimensions
MIOut = squeeze(sum(sum(miterms,1),2))';
CIEOut = squeeze(sum(sum(miterms_CIE,1),2))';
